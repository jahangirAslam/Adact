...
import { getContact, updateContact } from "../requests";
import { useEffect } from "react";


const formName = "editContact";
...
const EditContact = (props) => {
  ...
  useEffect(() => {
    makeRequest(setLoader, getContact, props.id, onContactSuccess, onContactError);
  ...
  const onFinish = (data) => {
    let payload = {
      "first_name": data.first_name, "last_name": data.last_name, "email": data.email, "landline": data.landline
      , "mobile": data.mobile, "id": props.id, "is_primary": data.is_primary
    }
    makeRequest(setLoader, updateContact, payload, onSuccess, onError);
  }

  const onSuccess = (data, res) => {
    notify("Contact Updated", res.msg);
  ...
  return (
    <ModalComponent mainTitle="Update" subTitle="Contact" visible={ true } footer={ footer } onCancel={ () => props.onUpdated(false) }>
      <Form
        layout="vertical"
        name={ formName }
        onFinish={ onFinish }
      ...
      >
        ...
        <Form.Item name="is_primary" label="Make it Primary :" className="da-mb-16">
          <Switch defaultChecked={ data.object.is_primary } />
        </Form.Item>
      </Form>
    </ModalComponent>
  );
